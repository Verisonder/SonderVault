...
import com.verisonder.sondervault.CrashLog
...
private sealed interface Where {
    ...
    data class Picking(val kind: com.verisonder.sondervault.media.PickKind) : Where
    data class Viewing(val index: Int) : Where
    data object Settings : Where
    data object Duress : Where
    data class DecoyPhotos(val decoy: com.verisonder.sondervault.vault.Vault) : Where
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        ...
        when (val here = where) {
            ...
            is Where.DecoyPhotos -> PickerScreen(
                here.decoy,
                com.verisonder.sondervault.media.PickKind.IMAGES,
...
private fun CrashReport(text: String, onDismiss: () -> Unit) {
    ...
    Column(
        ...
        Text("SonderVault closed unexpectedly", style = MaterialTheme.typography.titleMedium)
