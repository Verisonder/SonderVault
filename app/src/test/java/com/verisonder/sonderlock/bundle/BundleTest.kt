...
class BundleTest {
    ...
    fun `editing the argon parameters is refused`() {
        ...
        val edited = File(base, "edited.sonderlock").apply { writeBytes(raw) }
    ...
    fun `a tampered entry is caught when it is read`() {
        ...
        val edited = File(base, "tampered.sonderlock").apply { writeBytes(raw) }
    ...
    fun `a truncated bundle is refused`() {
        ...
        val written = write(listOf(item))
        val raw = written.file.readBytes()
        val cut = File(base, "cut.sonderlock").apply { writeBytes(raw.copyOf(raw.size - 500)) }
    ...
    fun `an unrelated file is refused`() {
        val junk = File(base, "junk.sonderlock").apply { writeBytes(bytes(2000, 14)) }
        var refused = false
        try {
            open(junk)
        } catch (e: IOException) {
